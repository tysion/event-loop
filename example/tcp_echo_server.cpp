...
struct TcpConnection : std::enable_shared_from_this<TcpConnection> {
  ...
  void HandleAsync() {
    const oxm::Event event = oxm::MakeEvent(socket_, {oxm::Event::Type::ReadWrite});
    event_id_ = loop_->Submit(event, [self = shared_from_this()](oxm::Event::Mask mask) {
      if (mask.HasError()) {
        self->OnError(mask);
        return;
      }
...
struct TcpAcceptor : std::enable_shared_from_this<TcpAcceptor> {
  ...
  void AcceptAsync() {
    const oxm::Event event = oxm::MakeEvent(socket_, {oxm::Event::Type::Read});
    event_id_ = loop_->Submit(event, [self = shared_from_this()](oxm::Event::Mask mask) {
      if (mask.HasError()) {
        self->OnError(mask);
        return;
